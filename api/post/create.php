...
$postParams = array(
    'name'=> sanitiseBasic($_POST["name"]),
    'description' => sanitiseBasic($_POST["description"]),
...
if (CSRF::validateToken($_POST['csrf-token'], $_SESSION['csrf-token-expire'])) {
    ...
    if($imageProcessSuccess && $image->create()) {
        ...
        if($post->create()) {
            ...
            redirectTo(
                'upload_form',
                'Please choose a new unique name and try again',
                null,
                $postParams,
                true,
                true,
...
function redirectTo($destination, $errorMessage, $successMessage, $postParams, $returnParams, $returnParamErrors, $append) {
    ...
    if ($returnParamErrors) {
        $paramErrors = "nameErr=true&descriptionErr=true";
